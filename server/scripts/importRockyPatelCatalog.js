/**
 * Import Rocky Patel catalog rows + processed single-cigar product images.
 *
 * Expects:
 *   assets/rocky-patel/catalog.json
 *   tmp/rocky-patel/blend-images.json  (blend name → local processed JPEG)
 *
 * Catalog rows use blend-only `name` + optional `size_name` (vitola), so Add Cigar
 * can list all sizes under one cigar name.
 *
 * Run from server/:
 *   node scripts/importRockyPatelCatalog.js
 *   node scripts/importRockyPatelCatalog.js --dry-run
 */
...
const ROOT = path.resolve(__dirname, '../../tmp/rocky-patel');
const CATALOG_PATH = [
  path.resolve(__dirname, '../../assets/rocky-patel/catalog.json'),
...
async function main() {
  if (!CATALOG_PATH) {
    console.error('Missing Rocky Patel catalog JSON (expected assets/rocky-patel/catalog.json)');
    process.exit(1);
  }
  ...
  if (!DRY_RUN) {
    ...
    // Reuse existing uploaded image URLs when possible (old rows had vitola in name).
    const existingImages = await pool.query(
      `SELECT name, image FROM cigar_catalog
       WHERE brand = 'Rocky Patel' AND COALESCE(image, '') <> ''`
    );
    ...
    const del = await pool.query(`DELETE FROM cigar_catalog WHERE brand = 'Rocky Patel'`);
    console.log(`Cleared ${del.rowCount} existing Rocky Patel rows`);
  }
      ...
      if (!imageByBlend.has(blend)) {
        ...
        if (localPath && fs.existsSync(localPath)) {
          if (DRY_RUN) {
            ...
            const storagePath = `catalog/${slugify(`rp_${blend}`)}_${Date.now()}.jpg`;
