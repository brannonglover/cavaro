...
function formatCigarCount(count) {
  const n = count ?? 0;
  return `${n} ${n === 1 ? 'Cigar' : 'Cigars'}`;
}

function formatHumidorMeta(humidor, { includeCount = true } = {}) {
  return [
    includeCount ? formatCigarCount(humidor.cigar_count) : null,
...
export function buildInventorySummary(humidors = [], selectedHumidorId = null) {
  if (humidors.length === 0) {
    return { title: 'Humidors', cigarCountLabel: null, metaParts: [] };
  }
  ...
  if (humidors.length === 1) {
    ...
    return {
      title: null,
      cigarCountLabel: formatCigarCount(humidor.cigar_count),
      metaParts: formatHumidorMeta(humidor, { includeCount: false }),
    };
  }
